  ...
  public class SqlFunctionsForNullValueTests
  {
    ...
    public void TestIsNullFunction()
    {
      ...
      using (IDbConnection conn = engine.GetConnection())
      {
        ...
        var ids = conn.Query<IdAndName>("SELECT Id, IsNull(Name, 'NO NAME GIVEN') AS 'Name' FROM [Sample] ORDER BY Name");
    ...
    public void TestNullIfFunction()
    {
      ...
      {
        conn.Execute(SampleTable);
        var ids = conn.Query<IdAndName>("SELECT Id, NullIf(Name, 'Gwen') AS 'Name' FROM [Sample]  ORDER BY Name");
        Assert.IsNotNull(ids);
        Assert.AreEqual(8, ids.Count());
